    ...
    public class JsonMapper
    {
        ...
        private static void AddTypeProperties (Type type)
        {
            ...
            foreach (PropertyInfo p_info in type.GetProperties ()) {
                ...
                var attributes = p_info.GetCustomAttributes(typeof(DataMemberAttribute), true);
            ...
            foreach (FieldInfo f_info in type.GetFields ()) {
                var attributes = f_info.GetCustomAttributes(typeof(DataMemberAttribute), true);
                ...
                while (true) {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
                        ...
                        if (prop_data.IsField) {
                            FieldInfo f_info = (FieldInfo)prop_data.Info;
                            ...
                            {
                                f_info.SetValue(
                                    instance, ReadValue(prop_data.Type, reader, f_info.GetValue(instance)));
                            }
                            else
                            {
                            ...
                            if (p_info.CanWrite && attributes.Length != 0)
                                p_info.SetValue (
                                    instance,
                                    ReadValue (prop_data.Type, reader, p_info.GetValue(instance)),
                                    null);
                            else
                                ReadValue (prop_data.Type, reader);
                        }
            ...
            Type json_type, Type value_type, ImporterFunc importer)
        {
         ...
         private static bool ignoreObj(object obj)
        {
            if (obj == null)
                return true;
            ...
            if (obj is Enum)
            {
                Type e_type = Enum.GetUnderlyingType(obj.GetType());

                if (e_type == typeof(int))
                {
                    if ((int)obj == 0)
                        return true;
                                        ...
                                        int depth)
        {
            ...
            foreach (PropertyMetadata p_data in props) {
                if (p_data.IsField) {
                    object obj_value = ((FieldInfo)p_data.Info).GetValue(obj);
                    if(!ignoreObj(obj_value))
                    {
                        writer.WritePropertyName(p_data.Info.Name);
                        WriteValue(obj_value,
                    ...
                    if (p_info.CanRead) {
                        object obj_value = p_info.GetValue(obj, null);
                        if (!ignoreObj(obj_value))
                        {
                            writer.WritePropertyName(p_data.Info.Name);
                            WriteValue(obj_value,
